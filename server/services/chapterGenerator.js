...
export class ChapterGenerator {
  ...
  async _buildPrompt(chapterId, chapterTitle, outline, references, partContext, maxTokens, estimatedTime, totalChapters, currentNum) {
    ...
    let timeConstraint = '';
    {
      ...
      const tokenCharLimit = Math.round(maxTokens * 1.5);
...
- **출력 토큰 ${maxTokens.toLocaleString()} 이내** (이 한도를 초과하면 응답이 잘립니다! 반드시 ${maxTokens.toLocaleString()} 토큰 이내로 마무리하세요)
...
다음 장에서는 ~를 배워볼 거예요. 기대되시죠?`;
    } else if (isClassPreview) {
      // 수업 프리뷰: 교사-학생 대화 기반 강의 자료
      docStructure = `# 수업 프리뷰 문서 구조 (필수 - ${effectiveTimeLabel} 차시용)

이 문서는 **교사가 수업을 미리 체험할 수 있는 완성형 강의 자료**입니다.
실제 교실에서의 대화, 실습, Q&A가 모두 포함되어 있어 이것만 읽으면 바로 수업할 수 있어야 합니다.

## 기본 정보
- **주제**: [이 주차의 핵심 주제]
- **시간**: ${effectiveTimeLabel}
- **형태**: [수업 형태: 2인 1조, 개인, 팀 등]
- **준비물**: [필요한 하드웨어, 소프트웨어, 교구 목록]

## 학습목표
1. ~할 수 있다 (행동 동사로 시작, 3~4개)

## 타임라인
교시별 세부 시간 배분을 볼드+목록으로 작성:
- **[N교시: OO분]** 큰 주제
  - 00-10분: 활동 1
  - 10-25분: 활동 2
  - ...
...
선생님: "학생 반응에 대한 교사의 피드백"

**[시각 자료 — 하드웨어 회로도]** (해당 시)

하드웨어 관련 시각 자료는 반드시 아래 HTML 마크업을 사용하세요.
ASCII art로 회로도를 그리지 마세요!

Pico 핀 배치도:
\`\`\`html
<div class="hw-diagram" data-type="pico-pinout"
     data-highlight='[{"pin":"핀이름","label":"용도설명","color":"색상코드"}]'>
</div>
\`\`\`
...
<div class="hw-diagram" data-type="connection"
     data-title="회로 제목"
     data-connections='[{"from":"Pico핀","to":"부품1","then":"부품2","color":"색상"}]'
...
소프트웨어/개념 다이어그램은 Mermaid를 사용하세요.

**[코드: 파일명.py]** (실습 코드가 있을 때)
...
- **Q**: "학생이 실제로 물어볼 질문"
- **A**: "학생 눈높이에 맞는 답변 (비유 활용)"

**[수업 장면]** (각 교시마다 최소 1개)

학생 이름을 사용한 구체적 시나리오:
- 문제 해결 과정, 협업, 유레카 모먼트, 실수와 극복
- 교사의 자연스러운 개입과 피드백
- 짧지만 생생하게, 수업을 '미리 볼 수 있게'

## 도전과제 (매 주차 마지막)

- **⭐ Level 1**: 기본 — 수업 내용을 약간 변형
- **⭐⭐ Level 2**: 응용 — 두 가지 개념 결합
- **⭐⭐⭐ Level 3**: 심화 — 창의적 확장
...
## 교사용 사전 준비 메모
- 수업 전 준비해야 할 것들 (장비, 소프트웨어, 자료 등)

## 절대 금지
- ASCII art로 회로도/핀배치/다이어그램을 그리지 마세요 — hw-diagram HTML 마크업을 사용하세요
- 마크다운 테이블(파이프+대시) 대신 볼드+목록을 사용하세요 (기본 정보 테이블만 예외 허용)`;
...
`;
    ...
    // ============================================================
    // 수업 프리뷰 전용 프롬프트 (교사-학생 대화 기반 강의 자료)
    // ============================================================
    if (isClassPreview) {
      return `당신은 ${pc.role}입니다.
교사가 이 문서만 읽으면 수업을 미리 체험하고 바로 진행할 수 있는 완성형 강의 자료를 작성하세요.
...
# 대상 독자
**${pc.audience}**
- 이 문서를 읽고 바로 수업을 진행할 현장 교사입니다
- 실제 교실에서 할 말, 학생 반응, 대처법이 모두 필요합니다
- 코드와 회로 연결은 복사해서 바로 사용할 수 있어야 합니다
...
## 강의 스크립트 (가장 중요!)
- 선생님의 발화문은 \`선생님: "..."\` 형식으로 실제로 교실에서 할 말을 작성하세요
- 학생 반응은 \`학생: "..."\` 또는 \`학생 A: "..."\` 형식으로 자연스럽게
- 교사 행동/지시는 (괄호) 안에 작성: (학생들 연결), (순회하며 확인), (화면 제시)
- 전문용어는 **비유로 먼저 설명** → 그 다음 정확한 용어 소개
- 교사가 이 스크립트를 그대로 읽어도 자연스러운 수업이 되어야 합니다

## 실습 코드 작성법
- 모든 코드는 **복사해서 바로 실행 가능**해야 합니다
- 코드 파일명을 명시: \`step1_내장led_켜기.py\`
- WHAT(무엇을) → WHY(왜) → HOW(코드) 순서로 설명
- **모든 줄에 한국어 주석** 필수
- 이전 코드에서 점진적으로 확장 (step1 → step2 → step3)
- print()로 실행 결과를 확인할 수 있게

## 하드웨어 시각 자료 (필수!)
- 회로 연결, 핀 배치 등은 반드시 \`<div class="hw-diagram">\` HTML 마크업 사용
- **ASCII art로 회로도를 절대 그리지 마세요!** (텍스트 문자로 그림을 그리는 것 금지)
- 소프트웨어 개념/흐름 다이어그램은 Mermaid 사용

## 예상 Q&A
- 각 주요 활동마다 최소 3개
- 실제 학생이 물어볼 법한 질문 (때로는 엉뚱한 것도 포함)
- Q: / A: 형식, 답변은 학생 눈높이

## 수업 장면 시나리오
- 각 교시마다 최소 1개의 구체적 시나리오
- 학생 이름 사용 (지훈, 서연, 민수, 유진, 하윤, 재민 등)
- 문제 해결, 협업, 실수와 극복, 유레카 모먼트 등
- 교사의 자연스러운 개입과 피드백
...
## 형식 규칙
- **톤앤매너**: ${pc.tone}
- **마크다운 테이블**: 기본 정보 테이블(주제/시간/형태/준비물)만 허용, 나머지는 볼드+목록
- **ASCII art 절대 금지**: 텍스트로 그림/도표/핀배치/회로도를 그리지 마세요!
- **hw-diagram 사용**: 하드웨어 다이어그램은 반드시 hw-diagram HTML 마크업 사용
- 이모지는 도전과제 레벨 표시와 요약 체크마크에만 사용

${guidelinesText}
# 마크다운 형식으로 완전한 주차 수업 자료를 작성해주세요.
위 구조를 **반드시 모두** 포함하되, 교사가 이것만 읽고 바로 수업할 수 있는 구체성이 최우선입니다.
강의 스크립트의 자연스러움과 실습 코드의 실행 가능성을 특히 신경 쓰세요.
${templateAddition}
`;
    }
...
`;
  ...
  _validateChapter(content, templateId, chapterInfo, charMin, charMax) {
    ...
    const checks = sectionChecks[templateId];
    if (checks) {
