...
//   2) LOCAL_MODE: `gh auth token` CLI 출력
//   3) LOCAL_MODE: 환경변수 GITHUB_TOKEN
router.post('/github', asyncHandler(async (req, res) => {
  ...
  //    (LOCAL_MODE는 googleId='local'으로 목 주입되므로 토큰은 당연히 없음 → 폴백 진입)
  if (req.user?.googleId && req.user.googleId !== 'local' && userStore) {
    const github = await userStore.getGitHubToken(req.user.googleId);
    if (github?.token) {
      token = github.token;
      tokenSource = `user(${github.username || req.user.googleId})`;
      tokenUsername = github.username;
    }
  }

  // 2) LOCAL_MODE: gh CLI에서 토큰 조회
  if (!token) {
    try {
  ...
  if (!token) {
    return res.status(400).json({
      message: 'GitHub 토큰을 찾을 수 없습니다. 아래 중 하나를 진행해주세요:\n'
        ...
        + '• (로컬 모드) 터미널에서 `gh auth login` 실행\n'
        + '• (로컬 모드) .env 파일에 `GITHUB_TOKEN=ghp_...` 추가 후 서버 재시작',
    });
  }
  ...
  if (result.success && result.site_url) {
    ...
    // UserStore에 프로젝트 배포 이력 저장 (웹 배포판 실제 유저에 한해 — LOCAL_MODE나 gh CLI 폴백은 건너뜀)
    if (userStore && req.user?.googleId && req.user.googleId !== 'local' && tokenSource?.startsWith('user(')) {
      try {
