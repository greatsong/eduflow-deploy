...
export default function Deployment() {
  ...
  const [githubLocalHint, setGithubLocalHint] = useState(false); // LOCAL_MODE + gh 미인증 안내
    ...
    apiFetch('/api/auth/github/status')
      .then(data => {
        if (data.connected) {
          ...
          // LOCAL_MODE에서 gh CLI 미인증 — OAuth 팝업 대신 CLI 안내 표시
    ...
    <div className="space-y-6">
      ...
      {status?.hasMkdocsYml && (
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          {/* GitHub 연동 안내 — LOCAL_MODE 미인증 시: gh CLI 안내 */}
          {!githubUser && githubLocalHint && (
            <div className="mb-4 p-4 bg-amber-50 rounded-xl border border-amber-200">
              ...
              <p className="text-xs text-amber-800 mb-3 leading-relaxed">
                로컬 모드에서는 Google OAuth 팝업이 아니라 <code className="px-1 bg-amber-100 rounded">gh</code> CLI를 사용합니다.
          ...
          {/* GitHub 연동 안내 — 웹 배포판(미연동 시): OAuth 연동 버튼 */}
          {!githubUser && !githubLocalHint && (
